...
public interface BasicDao {
	...
	/**
	 * delete an entity. note: you must load the entity first or do like this:
	 * 
	 * <pre>
	 * User user = new User();
	 * user.setId("C0001");
	...
	/**
	 * load all entities according to a condition, which the condition come from the
	 * template. for example: I want to load all of the items of an order, then the
	 * template is the orderItem and set the orderId of the template. Then it will
	 * load all the items of this order.
	 * 
	 * @param template just an empty object to know which class, and set the
	 *                 condition to it.
