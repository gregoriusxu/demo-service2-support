...
public class ReferenceFactoryForList<S extends Serializable, T extends Entity<S>> {
	 ...
	 * @param list
	 */
	public void build(Ref ref, List<Entity<?>> list) {
		this.ref = ref;
	...
	private void loadOfOneToOne(Ref ref) {
		if (ref == null || entities == null || entities.isEmpty())
			return;
		...
		for (Entity<S> enj : listOfEntitiesNeedRef)
			mapOfEntitiesNeedRef.put(enj.getId(), enj);
	...
	private Method getMethod(Object service, String name) {
		if (name == null || name.isEmpty())
			throw new OrmException(
					"The method name is empty![service:" + service.getClass().getName() + ", method:" + name + "]");
