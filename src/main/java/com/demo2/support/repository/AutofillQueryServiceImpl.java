...
/**
 * The implement of the query service that it auto fill each of the object
 * property of the item of the query result set, like this: fill the addresses
 * of each the customer of the result set.
 * 
 * @author fangang
 */
public class AutofillQueryServiceImpl extends QueryServiceImpl {
	...
	protected ResultSet afterQuery(Map<String, Object> params, ResultSet resultSet) {
		...
		if (listOfJoins != null && !listOfJoins.isEmpty()) {
			for (Join join : listOfJoins) {
				// TODO have exception
				autoFillJoin((List<Entity<?>>) list, join);
			}
		}
		...
		if (listOfRefs != null && !listOfRefs.isEmpty()) {
			for (Ref ref : listOfRefs) {
				// TODO have exception
				autoFillRef((List<Entity<?>>) list, ref);
			}
		}
	 ...
	 * @param join
	 */
	private <S extends Serializable, T extends Entity<S>> void autoFillJoin(List<Entity<?>> list, Join join) {
	 ...
	 * @param ref
	 */
	private <S extends Serializable, T extends Entity<S>> void autoFillRef(List<Entity<?>> list, Ref ref) {
